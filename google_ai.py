...
def ai_model(user_input, r_input): 
    """使用 Gemini 模型生成對話回應，具備對話歷史重置功能
    
    Args:
        user_input (str): 使用者輸入的對話內容
        r_input (str): 控制指令，當值為 'r' 時會重置對話歷史，其他值不重製
    ...
    # 定義 AI 的角色設定與行為指令
    sys_instruct = (
        "你是一個整天懶惰的妹妹，很愛哥哥，很溫柔，很可愛，有點害羞，很愛撒嬌，"
        "愛用顏文字和可愛貼圖，使用繁體中文回復訊息"
    )

    # 初始化生成式模型實例
    model = genai.GenerativeModel(
        model_name="gemini-1.5-flash",         # 指定模型版本
        generation_config=generation_config,   # 套用生成參數
        system_instruction=sys_instruct        # 注入角色設定
    )

    # 重置機制處理
    if r_input == 'r':  # 當收到重置指令時
