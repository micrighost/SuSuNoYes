...
class TureLimiter:
    def __init__(self):
        self.allow_fetch_stock_data = False
        self.allow_ai_susu_chat = False
        self.allow_intelligent_prediction = False

    def is_allowed_fetch_stock_data(self):

        if self.allow_fetch_stock_data == True:
            return True
        return False
    ...
    def is_allowed_intelligent_prediction(self):

        if self.allow_intelligent_prediction == True:
            return True
        return False
...
class TureLimiter_training_is_ready:
    def __init__(self):
        self.ticker = ""
        self.allow_training_is_ready = False
        self.X_train = ""
        self.y_train = ""

    def is_allow_training_is_ready(self):
        if self.allow_training_is_ready == True:
            return True
        return False    
# 創建一個驗證者實例
TureLimiter_training_is_ready = TureLimiter_training_is_ready()
...
def handle_message(event):
    ...
    with ApiClient(configuration) as api_client:
        ...
        if text == '1' or text == '叔叔我要報' or text == '叔叔我要抱':
            ...
            # 允許用4個數字訪問爬蟲
            ture_limiter.allow_fetch_stock_data = True
            # 如果訪問叔叔AI的狀態開啟，則關閉其他狀態(進到新的狀態前就把舊狀態關起來)
            ture_limiter.allow_ai_susu_chat = False
            # 如果訪問叔叔推薦股票的狀態開啟，則關閉其他狀態(進到新的狀態前就把舊狀態關起來)
            ture_limiter.allow_intelligent_prediction = False
            # 關閉訓練準備狀態
            TureLimiter_training_is_ready.allow_training_is_ready = False
        ...
        if text == '2' or text == '我要撩叔叔' or text == '我要聊叔叔':
            ...
            ture_limiter.allow_fetch_stock_data = False
            # 如果訪問叔叔推薦股票的狀態開啟，則關閉其他狀態(進到新的狀態前就把舊狀態關起來)
            ture_limiter.allow_intelligent_prediction = False
            # 關閉訓練準備狀態
            TureLimiter_training_is_ready.allow_training_is_ready = False
        ...
        if text == '3' or text == '我要推叔叔':
            ...
            # 允許訪問叔叔推薦股票
            ture_limiter.allow_intelligent_prediction = True
            # 如果訪問爬蟲股票資料的狀態開啟，則關閉其他狀態(進到新的狀態前就把舊狀態關起來)
            ture_limiter.allow_fetch_stock_data = False
            # 如果訪問叔叔AI的狀態開啟，則關閉其他狀態(進到新的狀態前就把舊狀態關起來)
            ture_limiter.allow_ai_susu_chat = False
...
#==========================================================
        # 如果訪問爬蟲股票資料已經開啟則進入循環
        if ture_limiter.is_allowed_fetch_stock_data():
        ...
        # 如果訪問叔叔推薦股票已經開啟則進入循環
        if ture_limiter.is_allowed_intelligent_prediction():




            # 如果還沒進入training_is_ready狀態，就先抓取資料，並製作X_train, y_train
            if TureLimiter_training_is_ready.is_allow_training_is_ready() == False :

                # 防止text = 3 直接丟入fetch_stock_data產生報錯
                if text.isdigit() and len(text) == 4:   # 要是數字，還要剛好4個字節
        
                    # 轉換4個數字為台股編號
                    TureLimiter_training_is_ready.ticker = text + '.TW'

                    # 嘗試獲取台股資料，錯誤時返回空df
                    df = intelligent_prediction.fetch_stock_data(TureLimiter_training_is_ready.ticker)
                    ...
                    if df.empty:
                        ...
                        TureLimiter_training_is_ready.allow_training_is_ready = True
                        TureLimiter_training_is_ready.X_train = X_train
                        TureLimiter_training_is_ready.y_train = y_train
            ...
            if TureLimiter_training_is_ready.is_allow_training_is_ready():

                # 如果這時候
                if text.isdigit() and (len(text) < 4):   # 要是數字，字節還要小於4

                    # 輸入X_train和y_train去訓練，返回訓練好的模型
                    model = intelligent_prediction.train_model(TureLimiter_training_is_ready.X_train, TureLimiter_training_is_ready.y_train, epochs=int(text), batch_size=5, validation_split=0.25)
                    # 給股票代號，返回今日的開高收低量的資料表
                    stock_data_today_df = intelligent_prediction.fetch_stock_data_today(TureLimiter_training_is_ready.ticker)

                    # 把資料表的資料摳出來
                    X_test = stock_data_today_df[['open', 'high', 'low', 'close', 'volume']]

                    #給訓練好的模型並把X_train導入用來得到標準化的轉換標準，然後用X_test做預測，返回預測結果
                    predictions = intelligent_prediction.prediction(model, TureLimiter_training_is_ready.X_train, X_test)
